...
class TestGroupManagementIntegration(TestCase):
    ...
    def test_complete_group_management_workflow(self):
        ...
        login_data = {
            'login': 'user1@example.com',  # Using email
            'password': 'password123'
        }
        ...
        user_usernames = [user['username'] for user in response.data]
        expected_usernames = ['user1', 'user2', 'user3', 'user4', 'user5']
        for username in expected_usernames:
            self.assertIn(username, user_usernames)
        
        # Add user2 to the group
        add_member_data = {
            'user_identifier': 'user2'  # Using username
        }
